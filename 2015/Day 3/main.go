...
func main() {
	var xSanta, ySanta int = 0, 0         // current house santa
	var xRoboSanta, yRoboSanta int = 0, 0 // current house robo santa
	var visitedHousesSanta = [][]int{
		{0, 0},
	}
	var visitedHousesRoboSanta = [][]int{
		{0, 0},
	}
	var onceVisitedHouses [][]int
	...
	for index, direction := range input {
		if index%2 == 0 {
			...
			visitedHousesRoboSanta = append(visitedHousesRoboSanta, []int{xSanta, ySanta})
		} else {
			switch direction {
			case '<':
				xRoboSanta -= 1
			...
			visitedHousesSanta = append(visitedHousesSanta, []int{xRoboSanta, yRoboSanta})
		}
	}

	var found bool
	for _, house := range append(visitedHousesSanta, visitedHousesRoboSanta...) {
		for _, visitedHouse := range onceVisitedHouses {
			if visitedHouse[0] == house[0] && visitedHouse[1] == house[1] {
				found = true
				break
			}
		}
		if !found {
			onceVisitedHouses = append(onceVisitedHouses, []int{house[0], house[1]})
		}
		found = false
	}

	fmt.Printf("Houses receive at least one present: %d", len(onceVisitedHouses))
}
